...
email_host_patterns["Cloudflare"]="*cloudflare.net"
...
email_host_patterns["Uniweb"]="mx1.pub.mailpod1-osl1.one.com|mx2.pub.mailpod1-osl1.one.com|mx3.pub.mailpod1-osl1.one.com|*.uniweb.no|*.fastname.no"
...
guess_email_host() {
    ...
    for host in "${!email_host_patterns[@]}"; do
        if echo "$mx_records" | grep -Eq "${email_host_patterns[$host]}"; then
            echo -e "${GREEN}Prboably hosted on $host.${RESET}"
            return
        fi
    done
...
# MX RECORD(S)
echo -e "${YELLOW}MX RECORD(S)${RESET}" "${CYAN}* Mail Exchange${RESET}"
...
# SPF RECORD
echo -e "${YELLOW}SPF RECORD${RESET}" "${CYAN}* Sender Policy Framework${RESET}"
...
if [ -n "$dmarc_result" ]; then
    echo -e "${YELLOW}DMARC RECORD${RESET}" "${CYAN}* DNS based authentication${RESET}"
...
if [ -n "$dkim_results" ]; then
    echo -e "${YELLOW}DKIM RECORD(S)${RESET}" "${CYAN}* DomainKeys Identified Mail${RESET}"
...
# Guessing Email Host
echo -e "${YELLOW}EMAIL GUESSER${RESET}" "${CYAN}* Dice rolled...${RESET}"
guess_email_host "$1"