...
show_help() {
    ...
    echo -e "${MAGENTA}------------------------------------------${RESET}"
    echo -e "${RED}Did you input an actual domain, or did you fat-finger?${RESET}"
    echo
    echo -e "${YELLOW}<check domain.tld>${RESET}" "${GREEN}checks the following:${RESET}"
    ...
    echo -e "${MAGENTA}* REGISTRAR:${RESET}" "${GREEN}Who is the registrar for the domain, alternatively the Registry (White Label)${RESET}"
    echo
    echo -e "${YELLOW}<check>${RESET}" "${BLUE}allows for flags so you pull only relevant info:${RESET}" "${CYAN}-all, -host -mail, -cert, -ssl${RESET}"
    echo
